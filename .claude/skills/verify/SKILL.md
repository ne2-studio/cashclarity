---
name: verify
description: "Selects and runs risk-based verification for CashClarity changes. Use before considering implementation work complete."
model: haiku
---
...
| Infra/CI/environment | Test or smoke proving wrappers, env vars, startup, health, connectivity | affected command; `backend-acceptance` or `frontend-acceptance` for containers | Dockerfile, compose, workflow, env contract, service wiring changed |
...
| Visual behavior | Functional assertions + inspected screenshot | command for the behavior | Browser layout, build output, responsive rendering |
...
```
backend  backend-persistence  backend-acceptance  frontend  frontend-acceptance  all
```
...
- Don't ignore service errors unless the scenario intentionally tests an error path.
